...
class Iterator {
 ...
 public:
  ...
  T& operator*() const { return current->data[index]; }
...
class Deque {
 ...
 public:
  ...
  Iterator<T> begin() {
    return Iterator(first != nullptr ? first : nullptr,
                    first != nullptr ? first->first : 0);
  }
  ...
  Iterator<T> erase(Iterator<T> pos) {
    ...
    // Если удаляемый элемент - единственный в узле
