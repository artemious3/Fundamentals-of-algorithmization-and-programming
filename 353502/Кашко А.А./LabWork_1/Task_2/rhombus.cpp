...
void Rhombus::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) {
    ...
    // Задаем координаты вершины ромба
    QPointF center(0, 0); // Центр ромба

    // Вычисляем координаты остальных вершин на основе центра и длин диагоналей
    QPointF point1(center.x() + diagonal1 / 2, center.y()); // Вершина 1
    QPointF point2(center.x(), center.y() - (diagonal1 - difr) / 2); // Вершина 2
    QPointF point3(center.x() - diagonal1 / 2, center.y()); // Вершина 3
    QPointF point4(center.x(), center.y() + (diagonal1 - difr) / 2); // Вершина 4
